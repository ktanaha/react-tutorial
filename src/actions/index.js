...
export const setPlace = place => dispatch => dispatch({
    type: 'CHANGE_PLACE',
    place,
  });
...
export const setHotels = hotels => dispatch => dispatch({
    type: 'CHANGE_HOTELS',
    hotels,
  });

export const setSortKey = sortKey => dispatch => dispatch({
...
export const startSearch = () => (dispatch, getState) => {
  geocode(getState().place)
    ...
    .catch(() => {
      dispatch(setErrorMessage('通信でエラーが発生しました。'));
    });
};
